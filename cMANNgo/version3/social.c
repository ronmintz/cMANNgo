...
#define PROTOTYPE_PROB_ALGORITHM "logistic"
#define PROTOTYPE_PROB_PARAMETER 10.0
...
void storeParameters(int runNum)
{
    ...
    fprintf(fp, "PROTOTYPE_PROB_ALGORITHM %s\n", PROTOTYPE_PROB_ALGORITHM);
    fprintf(fp, "PROTOTYPE_PROB_PARAMETER %f\n", PROTOTYPE_PROB_PARAMETER);
...
void pretraining(FILE *fp)
{
    ...
    for (a = 0 ; a < N_AGENTS ; a++)
    {
        ...
        if (a == 0) createExampleSet(inputs, inputs);
...
//        p = 0.1;
    ...
    if ( strcmp(PROTOTYPE_PROB_ALGORITHM, "linear") == 0 )
    {
        p = 1.0 - x;
    }
    else if ( strcmp(PROTOTYPE_PROB_ALGORITHM, "logistic") == 0 )
    {
        x -= 0.5;
        p =  1.0 / (1.0 + exp(-PROTOTYPE_PROB_PARAMETER * x)); // prob of not using proto
        p =  1.0 - p;
    }
    else
    {
        printf("INVALID PROTOTYPE_PROB_ALGORITHM");
        exit(1);
    }

//  printf("prob of using prototype for input at tick %d = %f\n", tick, p);
//  printf("computed from %s algorithm using parameter %f\n", PROTOTYPE_PROB_ALGORITHM, PROTOTYPE_PROB_PARAMETER);
